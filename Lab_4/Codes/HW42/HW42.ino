...
const float SINE_FREQ_1 = 5.34;     // First sine wave's frequency to be generated. Hz.
const float SINE_FREQ_2 = 74.645;   // Second sine wave's frequency to be generated. Hz.

const int SINE_GEN_FREQ = 500;    // Frequency of the sine wave generation system. Hz.
const float SINE_AMP = 1.25;      // Sine wave's amplitude to be generated. Volts.
...
// Generates signal
void PWM_DT() {
DUTY_CYCLE = SINE_AMP_A * (sin(SINE_COEFF_1 * SINE_GEN_COUNTER) + sin(SINE_COEFF_2 * SINE_GEN_COUNTER)) + SINE_OFFSET_A;
